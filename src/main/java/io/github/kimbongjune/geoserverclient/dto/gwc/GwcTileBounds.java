 ...
 * These are tile indices, not geographic coordinates.</p>
 */
public class GwcTileBounds {
    ...
    /** Parses the server's {@code {{...}, {...}}} plain-text response. */
    ...
    /** Returns the coverage entry for the given zoom level, or null if not present. */
    public long[] forZoomLevel(int zoom) {
        for (long[] c : coverages) {
            if (c[4] == zoom) return c;
        }
        return null;
    }
    ...
    public boolean equals(Object o) {
        ...
        if (coverages.size() != that.coverages.size()) return false;
        for (int i = 0; i < coverages.size(); i++) {
            if (!Arrays.equals(coverages.get(i), that.coverages.get(i))) return false;
        }
        return true;
    }
