...
/**
 * DTO for WMTS (cascading) store details.
 *
 * <p>Maps the response body of {@code GET /rest/workspaces/{ws}/wmtsstores/{name}}.</p>
 *
 * <p>Verified against GeoServer 2.28.2. Includes fields not documented in Swagger.</p>
 *
 * <p><b>Key difference from WmsStore:</b> the sub-resource link uses the key {@code "layers"},
 * not {@code "wmslayers"}.</p>
 */
...
public class WmtsStore {
    ...
    /** Sub-resource link. Unlike WmsStore which uses "wmslayers", the key here is "layers". */
    @JsonProperty("layers")
    ...
    @Override
    public int hashCode() {
        return Objects.hash(name, description, enabled, defaultStore, workspace, capabilitiesURL, user, password, authKey, headerName, headerValue, maxConnections, readTimeout, connectTimeout, disableOnConnFailure, dateCreated, dateModified, layers, metadata);
    }
